    ...
    !!!!read hod data and setting here!!!!
    write(*,*) 'hod : start reading file'
    open(unit=100,file='/home/hyzhang/projects/hod/cosmomc-hod/data/hod/setting.dat')
    read(100,*) nhalos,nsates,timesRans,numShells,r_max,r_min,v_r
    write(*,*) nhalos,nsates,timesRans,numShells,r_max,r_min,v_r
    close(100)
    allocate(mass(nhalos))
!    allocate(rvir(nhalos))
!    allocate(rs(nhalos))
    allocate(pos_h(nhalos))
    allocate(pos_s(nsates))
    allocate(num_sat(nhalos))
    allocate(ind_sat(nhalos))
    use_2pcf=.true.
    use_3pcf=.false.
    open(unit=200,file='/home/hyzhang/Documents/data/hod/MDhalos.txt')
    open(unit=300,file='/home/hyzhang/Documents/data/hod/sate_catalog/sate_ind.dat')
    write(*,*) '200 300 start'
    do ii=1,nhalos
    read(200,*) mass(ii),dummy2,dummy2,pos_h(ii)
    read(300,*) dummy2,num_sat(ii),dummy2,ind_sat(ii)
    end do
    close(200)
    close(300)
    open(unit=400,file='/home/hyzhang/Documents/data/hod/sate_catalog/sate_pos.dat')
    write(*,*) '400 start'
    do ii=1,nsates
    read(400,*) dummy2,pos_s(ii)
    end do
    close(400)
    V_box=v_r**3
    ...
    dummy2 = corr%getShells(shells)
    dummy2 = corr%getTriangles(triangles)
    if(use_2pcf) then
    allocate(twopcf_data(numShells))
    allocate(cov2(numShells,numShells))
    allocate(invcov2(numShells,numShells))
    allocate(work2(1000))
    open(unit=500,file='/home/hyzhang/Documents/data/hod/mock_MD_run1/2pcf_rerun/twopoint_0199.dat')
    open(unit=600,file='/home/hyzhang/Documents/data/hod/mock_MD_run1/cov2/2pcf_cov.dat')
    write(*,*) '500 600 start'
    do ii=1,numShells
        read(500,*) dummy2, twopcf_data(ii)
        read(600,*) cov2(ii,1:numShells)
!    if(ii==1) write(*,*) cov2(ii,:)
    end do 
    close(500)
    close(600)
    invcov2=cov2
    call dvert(invcov2,numShells,numShells,work2)
!    write(*,*) invcov2
    end if
    if(use_3pcf) then
    allocate(threepcf_data(numTriangles))
    allocate(cov3(numTriangles,numTriangles))
    allocate(invcov3(numTriangles,numTriangles))
    allocate(work3(10000))
    write(*,*) '700 800 start'
    open(unit=700,file='/home/hyzhang/Documents/data/hod/mock_MD_run1/3pcf_rerun/threepoint_0199.dat')
    open(unit=800,file='/home/hyzhang/Documents/data/hod/mock_MD_run1/cov3/3pcf_cov.dat')
    do ii=1,numTriangles
        read(700,*) dummy2,dummy2,dummy2,threepcf_data(ii)
        read(800,*) cov3(ii,1:numTriangles)
    end do
    close(700)
    close(800)
    invcov3=cov3
    call dvert(invcov3,numTriangles,numTriangles,work3)
!    write(*,*) invcov3
    end if
    write(*,*) 'data input complete'
    !!!!end hod part!!!!
    call Setup%Init()
